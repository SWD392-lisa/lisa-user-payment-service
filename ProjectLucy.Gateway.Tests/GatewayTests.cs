...
namespace ProjectLucy.Gateway.Tests
{
    ...
    public class GatewayTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        // repo-service không có auth riêng -> gateway phải chặn bằng JWT (policy "authenticated").
        [Fact]
        public async Task RepoRoute_WithoutJwt_ReturnsUnauthorized()
        {
            var client = _factory.CreateClient();

            var response = await client.GetAsync("/api/repos/anything");

            Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
        }

        // Địa chỉ downstream phải được override từ biến môi trường lúc khởi tạo.
        [Fact]
        public void EnvConfig_ShouldOverrideDownstreamAddresses()
        {
            const string expectedApiUrl = "http://my-api-server.com/";
            const string expectedRealtimeUrl = "http://my-realtime-server.com/";

            try
            {
                Environment.SetEnvironmentVariable("API_SERVICE_URL", expectedApiUrl);
                Environment.SetEnvironmentVariable("REALTIME_SERVICE_URL", expectedRealtimeUrl);

                using var factory = new WebApplicationFactory<Program>();
                var config = factory.Services.GetRequiredService<IConfiguration>();

                Assert.Equal(expectedApiUrl, config["ReverseProxy:Clusters:api-cluster:Destinations:api-1:Address"]);
                Assert.Equal(expectedRealtimeUrl, config["ReverseProxy:Clusters:realtime-cluster:Destinations:realtime-1:Address"]);
            }
            finally
            {
                Environment.SetEnvironmentVariable("API_SERVICE_URL", null);
                Environment.SetEnvironmentVariable("REALTIME_SERVICE_URL", null);
            }
        }

        // Có JWT hợp lệ -> gateway forward request và bơm header X-User-* + X-Correlation-ID xuống downstream.
        [Fact]
        public async Task HeaderTransform_WithValidJwt_ProxiesAndForwardsHeaders()
        {
        ...
        // Client tự bơm X-User-* phải bị gateway xóa (chống spoof) khi không có JWT.
        [Fact]
        public async Task SpoofedIdentityHeaders_AreStripped()
        {
            var mockHandler = new MockHttpMessageHandler();
            ...
            var client = factory.CreateClient();
            var request = new HttpRequestMessage(HttpMethod.Get, "/api/agora/token?channelName=test");
            request.Headers.Add("X-User-Id", "attacker-999");

            var response = await client.SendAsync(request);

            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.NotNull(mockHandler.LastRequest);
            Assert.False(mockHandler.LastRequest!.Headers.Contains("X-User-Id"));
        }
